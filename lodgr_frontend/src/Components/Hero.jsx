...
function Hero() {
    ...
    return(
        <div className="relative h-[500px] md:h-[600px] py-3 mx-4 overflow-hidden rounded-3xl bg-[#c] z-0">
            {heroImages.map((image,index) => (
                <div
                    key={index}
                    className={cn(
                        "absolute inset-0 bg-cover bg-center transition-opacity duration-500", currentSlide === index ? "opacity-100" : "opacity-0",
                    )}
                    style={{backgroundImage: `url(${image})`}}
                >    
                    <div className="absolute inset-0 bg-[#2d3748]/40"></div>
                </div>
            ))}
            ...
            <div className="relative z-10 flex flex-col items-center text-white justify-center h-full px-4 sm:px-8 z-0">
                ...
                <p className="text-base md:text-lg mb-8 text-center max-w-2xl">
                    Describe your dream destination, and let our AI find the perfect staycation for you.
                </p>
                    ...
                    <div className="relative flex items-center">
                        <div className="relative grow">
                            <input 
                                ...
                                name="search"
                                placeholder="Search..."
                                className="bg-[#2d3748] h-12 text-sm sm:text-base placeholder:text-white/70 placeholder:text-sm sm:placeholder:text-base sm:placeholder:content-['Describe_your_destination...'] border-0 rounded-full py-6 pl-4 pr-12 sm:pr-32 w-full transition-all"
                            />
                        </div>
                        
                        <button 
                            type="submit"
                            className="absolute right-2 h-[80%] my-auto bg-[#2d3748] text-white rounded-full px-2 sm:px-4 flex items-center gap-x-2 border-white border-2 hover:bg-white/10 hover:border-[#5eead4] hover:text-[#5eead4] transition-colors"
                        >
